...
COMPARE 
        ...
        CMP.B   SHIFT_OP,D7 *See if the first 4 bits matches the first 4 bits of the Shift Op Codes
...
SHIFT
        ...
        CMP.B   #$6,D7  
        BEQ     MEM_ROL_OUT *If equal, the opcode starts with E6, so it is a memory ROL
        CMP.B   #$7,D7 
        BEQ     MEM_ROR_OUT *If equal, the opcode starts with E7, so it is a memory ROR
        CMP.B   #$3, D7 
...
W_OR_L
        ...
        CMP.B #$8, D7    *If the register bits are 000, the word will be 1000, so it is a .W
        BEQ   WORD_OUT
        CMP.B #$9, D7    *If the register bits are 001, the words will be 1001, so it is a .L 
        BEQ   LONG_OUT
WORD_OUT
